...
This IC controls a motor operation by PWM constant current control. The peak cur rent value (setting current value) can be 
...
If the constant current control function is disabled, the RS pin shoul d be connected to GND, and th e voltage (1 to 5V) is 
input to VREF pin. 
