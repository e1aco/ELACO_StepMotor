...
UVLO hysteresis voltage Vhys_uvlo ― ― 200 ― mV 
...
then turns off the output transistors. In order to avoid malfunction by switching etc.,  detection mask time is prepare d inside 
IC. Since the operating temperature of TSD circuit has a hysteresis width, the IC returns automatically when the j unction 
...
Note 3: Over current detection (ISD)
  *Latch operation 
