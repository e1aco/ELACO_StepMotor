 ...
 ********/
static unsigned char MT6816_EvenParity(uint16_t data)
{
...
MT6816_ANGLE_T *USR_MT6816_ReadAngle(void)
{
    ...
    {
        /* ANGLE 寄存器通过帧 1 读取 */
        rx_angle = DRV_MT6816_SpiTransfer(tx_angle);
        rx_raw = DRV_MT6816_SpiTransfer(tx_raw);

        g_mt6816_st.raw_data =
            ((rx_angle & 0x00FF) << 8) | (rx_raw & 0x00FF);

        if (0 == MT6816_EvenParity(g_mt6816_st.raw_data))
        {
